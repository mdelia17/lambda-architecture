...
def foreach_batch_function(df, epoch_id):
    try:
        ...
        # create table if not exists dns.window_message_types (start timestamp, end timestamp, type text, count int, primary key (start, end, type));
        # SELECT * FROM dns.window_message_types;
        # SELECT start, end, type, count  FROM dns.window_message_types WHERE start='2021-07-24 08:23:00' AND end='2021-07-24 08:23:20' AND type='Standard query';
        # TRUNCATE dns.window_message_types; 
        
        print("sono prima del ciclo")
        for elem in aggregate_stream.collect():
            ...
            address_lookup_stmt = session.prepare("SELECT start, end, type, count  FROM dns.window_message_types WHERE start=? AND end=? AND type=?")
            ...
            session.execute("INSERT INTO dns.window_message_types (start, end, type, count) VALUES (%s, %s, %s, %s)", (elem[0], elem[1], elem[2], int(elem[3])))
            # se l'elem stava nel db viene fatto un inserimento con i campi aggiornati
            for row in rows: 
                # print(row)
                new_count = row.count + int(elem[3])
                # print(new_count)
                session.execute("INSERT INTO dns.window_message_types (start, end, type, count) VALUES (%s, %s, %s, %s)", (elem[0], elem[1], elem[2], new_count))
    except: 
        pass
